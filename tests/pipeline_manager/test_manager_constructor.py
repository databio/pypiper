...
def pytest_generate_tests(metafunc):
    ...
    if "spec_type" in metafunc.fixturenames:
        metafunc.parametrize(argnames="spec_type", argvalues=["cmdl", "ctor"])


@named_param("checkpoint_type", argvalues=["curr_checkpoint", "prev_checkpoint"])
def test_manager_starts_in_null_checkpoint_state(get_pipe_manager, checkpoint_type):
...
class ManagerConstructorCheckpointSpecificationTests:
    ...
    def test_just_stop(self, get_pipe_manager, spec_type, stop_type, stop_point):
    ...
    def test_complementary_specification_modes(
        ...
        args = argparse.Namespace(**cmdl_kwargs)
        pm = get_pipe_manager(name="complementary-test", args=args, **raw_kwargs)
        assert start_point == pm.start_point
        assert stop_point == getattr(pm, stop_type)
    ...
    def test_command_line_beats_constructor_keyword(
        ...
        cmdl_kwargs = {cp_spec: cmdl_values[cp_spec] for cp_spec in check_specs}
        ctor_kwargs = {cp_spec: ctor_values[cp_spec] for cp_spec in check_specs}
        args = argparse.Namespace(**cmdl_kwargs)
